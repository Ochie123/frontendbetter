...
function getStyles(name, CarSpecificationss, theme) {
  return {
    fontWeight:
    CarSpecificationss.indexOf(name) === -1
...
const AuctionCreateForm = props => {
  ...
  const [carSpecificationss, setCarSpecifications] = useState([]);
  ...
  function getStyles(name, selectedSpecificationss, theme) {
    return {
      fontWeight: selectedSpecificationss.indexOf(name) !== -1
        ? theme.typography.fontWeightMedium
        : theme.typography.fontWeightRegular,
    };
  }
  ...
  const handleSpecificationChange = selectedOptions => {
    setSelectedSpecifications(selectedOptions);
  };
    ...
    >
      ...
      <FormControl sx={{ m: 1, width: 300 }}>
        ...
        <Field
          ...
          multiple
          name="carSpecificationss"
          input={<OutlinedInput id="select-multiple-chip" label="Chip" />}
          onBlur={formikProps.handleBlur}
          error={formikProps.touched.carSpecificationss && Boolean(formikProps.errors.carSpecificationss)}
          helperText={formikProps.touched.carSpecificationss && formikProps.errors.carSpecificationss}
          variant="outlined"
          ...
          onChange={(event) => {
            formikProps.setFieldValue('carSpecificationss', event.target.value);
          }}
          value={formikProps.values.carSpecificationss || []}
        >
          {carSpecifications.map((car_specification) => (
            <MenuItem
              key={car_specification.id}
              value={car_specification.id}
              style={getStyles(car_specification, formikProps.values.carSpecificationss, theme)}
            >
